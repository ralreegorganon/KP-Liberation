...
switch (_notiftype) do {
	...
	case 2: {
		...
		waitUntil {
			secondary_objective_position_marker distance zeropos > 1000
		};
	...
	case 4: {
		["lib_intel_convoy", [markerText ([10000, _obj_position] call KPLIB_fnc_getNearestSector)]] call BIS_fnc_showNotification;
	};
	case 5: {
	...
	default {
		[format ["remote_call_intel.sqf -> no valid value for _notiftype: %1", _notiftype], "ERROR"] remoteExecCall ["KPLIB_fnc_log", 2];
	};
};